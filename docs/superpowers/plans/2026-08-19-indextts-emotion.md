...
> **2026-08-19 更新（Task 7 已自动化验证部分）：** 下方清单 1-9 中，代码路径/合成成功已程序验证
> （见 docs/TTS_SETUP.md「端到端验证」），标 `[自动✓]` 的项无需重复；**未标者仍需用户真实 GUI 验证**：
> 人耳听感（情感差异/语气）、真实音频设备、任务管理器显存复核。

1. 打开《魔天记》→ 点播放 → 首次 IndexTTS 加载 30-60s，状态栏显示加载中 → 自动开始朗读 `[自动✓ 加载/合成路径，加载实测 264s；状态栏文案需 GUI]`
2. 情感效果：同一句切换 平静/悲伤/激昂 → 明显差异 `[自动✓ 向量路径合成成功；听感需用户]`
3. 强度滑条：拉低 → 情感减弱；拉高 → 增强 `[自动✓ 参数透传有测试；听感需用户]`
4. 自动情感：读"快躲起来！"应有紧张语气（use_emo_text）`[自动✓ 合成成功（修复 use_qwen_emo 后）；语气需用户]`
5. 引擎切换：播放中切 edge-tts → 立即换引擎继续读 `[自动✓ smoke 不崩；真实音频连续性需用户]`
6. 显存：任务管理器确认 IndexTTS 运行时专用 GPU 内存 < 7.5GB `[自动✓ nvidia-smi 实测 ~6.9GB；建议任务管理器复核]`
7. 速度：单句合成 ~2s，预取使播放不中断 `[自动✓ 稳态 2.4s/句；播放连贯性需用户]`
8. 回退：卸载模型/显存不足 → 自动回退 edge-tts 并提示 `[自动✓ 回退逻辑有测试；UI 提示需用户]`
9. 回归：edge-tts 模式全部原功能正常 `[自动✓ smoke 跑通；真实音频需用户]`
