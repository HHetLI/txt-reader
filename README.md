...
- **单栏阅读界面**：正文占满主窗口，无侧栏干扰；**深色 / 护眼绿 / 浅色**三套阅读主题可切换
- **中文排版**：正文每段首行缩进 2 字符 + 段间距，符合中文小说阅读习惯
- **自动分章**：打开 txt 自动检测编码（UTF-8/GBK 等），识别「第 X 章 / 序章 / 楔子 / 番外」等标题分章
- **章节导航（菜单栏）**：「章节」菜单提供搜索跳转（`Ctrl+G`）、上一章/下一章；正文底部也有章末导航链接；支持上千章的长篇
- **正文搜索（Ctrl+F）**：输入即高亮全部匹配，`Enter`/`Shift+Enter` 或按钮循环跳转，显示匹配计数
- **播放句子跟读**：朗读时正文中的当前句子以高亮色逐句推进，播放栏显示句级进度（3/45 句）
- **系统托盘**：关闭窗口最小化到托盘，听书后台继续播放；托盘菜单控制播放/切章/退出
- **双 TTS 引擎**：
...
- **最近打开**：文件菜单列出最近 5 本，一键重开；重启后恢复上次窗口大小/位置
- **阅读设置**：字号/行距可调（快捷键）
